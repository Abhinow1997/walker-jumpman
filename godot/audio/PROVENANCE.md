...
`extract_boss_music.py` cannot put it back on a fresh machine either. There is
no import cache for it under `.godot/` either. What a clone gets is `music.gd`
warning and leaving the coast loop playing under **both** boss fights — the
isles' dragon and the Roost's Dragon Lord alike. Producing the `.ogg` with the
command below is the whole fix and needs no `.gitignore` change: `.ogg` is
